...
export function BlockStart({ position = [ 0, 0, 0 ] })
{
    return <group position={ position }>
            <Float floatIntensity={ 0.25 } rotateOnAxis={ 0.25 }>
                <Text 
...
/**
 * Crea un bloque con un spinner
 * @param {Array3} Posicion de la cuestión 
 * @returns {<group>}
 */
export function BlockSpinner({ position = [ 0, 0, 0 ] }) 
{
    const obstacle = useRef()
    const [ speed ] = useState(() => (Math.random() + 0.2) * (Math.random() < 0.5 ? -1 : 1)) // si el componente se recarga mantiene el speed, pero es distinto para cada componente
...
/**
 * Crea un bloque con un limbo
 * @param {Array3} Posicion de la base del bloque
 * @returns {<group>}
 */
export function BlockLimbo({ position = [ 0, 0, 0 ] }) 
{
    const obstacle = useRef()
    const [ timeOffset ] = useState(() => Math.random() * Math.PI * 2) // si el componente se recarga mantiene el speed, pero es distinto para cada componente
...
export function Level({ count = 5, types = [ BlockSpinner, BlockAxe, BlockLimbo ], seed = 0 })
{
    const blocks = useMemo(() => 
    {
        ...
        return blocks 
    }, [count, types, seed ]) // set the depenendencies to count and types. Si cambian se llama de nuevo a la función
